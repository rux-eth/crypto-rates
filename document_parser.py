...
from errors import (InvalidTradeException, MissingHeaderElementError,
...
class DocumentParser(object):
    ...
    def _validate_header(self):
        for field in list(self.required_fields.keys()):
            if field not in list(self.header.keys()):
    ...
    def xlsx_parser(self, reader):
        ...
        errors = []
        for row in reader:
            dict_row = {self.decode(k): self.decode(v) for k,v in list(row.items())}
            try:
    ...
    def parse_trade(self, row):
        trade = {}
        for native_name, vendor_name in list(self.header.items()):
            if not vendor_name:
                continue
    ...
    def validate_trade_type(self, trade):
        if trade['type'] not in ['buy', 'sell', 'deposit', 'withdrawal']:
    ...
    def validate_row(self, row):
        for key, _type in list(self.required_fields.items()):
