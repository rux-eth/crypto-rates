    ...
    /******/ [
        ...
        /***/ function (module, __webpack_exports__, __webpack_require__) {
            ...
            const unmatchedDisposal = (report, disposal, localCurrency) => {
                ...
                reportToUpdate = report.updateIn([disposalYear, 'unmatched'], (list) =>
                    list.push(sale)
                );

                if (disposal.isLost) {
                    reportToUpdate = reportToUpdate.updateIn([disposalYear, 'lost'], (list) =>
                        list.push(sale)
                    );
                } else if (disposal.isCompoundLiquidated) {
                    reportToUpdate = reportToUpdate.updateIn(
                        [disposalYear, 'compound_liquidations_borrower'],
                        (list = Object(external_immutable_['List'])()) => list.push(sale)
                    );
                } else {
                ...
                return Object(external_immutable_['Map'])({
                    ...
                    disposal: disposal
                        .set('assetAmount', new external_bignumber_js_['BigNumber'](0))
                        .set('proceedsAmount', new external_bignumber_js_['BigNumber'](0)),
                });
            };
            const exhaustLot = (report, disposal, lotStack, localCurrency) => {
                ...
                const disposalMoment = external_moment_default.a.utc(disposal.unix, 'X');
                const timeDiff = disposalMoment.diff(
                    external_moment_default.a.utc(lotToDiminish.unix, 'X')
                );
                ...
                if (disposal.isLost) {
                    currentReport = currentReport.updateIn([disposalYear, 'lost'], (list) =>
                        list.push(sale)
                    );
                } else if (disposal.isCompoundLiquidated) {
                    ...
                    currentReport = currentReport.updateIn([disposalYear, reportCategory], (list) =>
                        list.push(sale)
                    );
                }
            ...
            const exhaustDisposal = (report, disposal, lotStack, localCurrency) => {
                ...
                if (disposal.isLost) {
                    currentReport = report.updateIn([disposalYear, 'lost'], (list) =>
                        list.push(sale)
                    );
                } else if (disposal.isCompoundLiquidated) {
                    currentReport = currentReport.updateIn(
                ...
                report.reduce((aggregatePriorSales, reportYear, year) => {
                    ...
                    if (unmatched.size > 0) {
                        const currentAndPriorSales = unmatched.reduce((red, unmatchedSale) => {
                            const existing = red.get(
                                unmatchedSale.get('asset'),
                                new external_bignumber_js_['BigNumber'](0)
                            );
                            return red.set(
                                unmatchedSale.get('asset'),
                                external_bignumber_js_['BigNumber'].sum(
                                    existing,
                                    unmatchedSale.get('asset_amount')
                                )
                            );
                        }, aggregatePriorSales);
                        currentAndPriorSales.map((amount, holdingAsset) => {
                            const keypath = [year, 'assets', holdingAsset, 'holdings'];
                            const existingHoldings = currentReport.getIn(
                                keypath,
                                new external_bignumber_js_['BigNumber'](0)
                            );
                            currentReport = currentReport.setIn(
                                keypath,
                                existingHoldings.minus(amount)
                            );
                        });
            ...
            const addIncomeToReport = (report, taxLot) => {
                ...
                if (taxLot.get('isIncome')) {
                    ...
                    reportToUpdate = report.updateIn([incomeYear, 'income'], (list) =>
                        list.push(income)
                    );
                }
            ...
            const updateReportIncomeFromLots = (report, lots, year) => {
                const unixSOY = Number(
                    external_moment_default.a.utc(year, 'YYYY').startOf('year').format('X')
                );
                const unixEOY = Number(
                    external_moment_default.a.utc(year, 'YYYY').endOf('year').format('X')
                );
            ...
            const setupAssetProperties = (year, report, yearDisposals, allLots) => {
                ...
                const reportWithIncrease = allLots.reduce((accReport, assetLots, assetKey) => {
                    ...
                    const assetReduction = assetLotsInYear.reduce((numReduction, lot) => {
                        return external_bignumber_js_['BigNumber'].sum(
                            lot.assetAmount,
                            numReduction
                        );
                    }, new external_bignumber_js_['BigNumber'](0));
                ...
                const reportWithDecrease = yearDisposals.reduce((accReport, disposal) => {
                    ...
                    const existingIncrease = accReport.getIn(
                        keypathIncrease,
                        new external_bignumber_js_['BigNumber'](0)
                    );
                    const existingHoldings = accReport.getIn(
                ...
                return yearList.map((v) => {
                    ...
                    return Object(external_immutable_['Map'])({
                        ...
                        disposals: sortedDisposals.filter(
                            (disposal) =>
                                external_moment_default.a.utc(disposal.unix, 'X').format('YYYY') ===
                                stringYear
                        ),
                    });
                });
            ...
            const updateReportHoldingsFromLots = (reportAndLots, year) => {
                ...
                reportAndLots.get('lots').map((value, lotAsset) => {
                    ...
                    if (assetLotsInYear.size > 0) {
                        ...
                        const currentHoldings = currentReport.getIn(
                            keyPath,
                            new external_bignumber_js_['BigNumber'](0)
                        );
                        currentReport = currentReport.setIn(
                            keyPath,
                            external_bignumber_js_['BigNumber'].sum(currentHoldings, assetHoldings)
                        );
                    }
                });
                return currentReport;
            ...
            const buildYearList = (sorted) => {
                const firstYear = Number(
                    external_moment_default.a.utc(sorted.get(0).unix, 'X').format('YYYY')
                );
                const lastYear = Number(
                    external_moment_default.a.utc(sorted.get(-1).unix, 'X').format('YYYY')
                );
                const distance = lastYear - firstYear + 1;
                return Object(external_immutable_['fromJS'])(
                    Array.from(
                        {
                            length: distance,
                        },
            ...
            const sortAccountingRecords = ({ records }) => {
                return records.sort((a, b) => a.unix - b.unix);
            };
            const sortDisposals = ({ records }) => {
                return records.sort((a, b) => a.unix - b.unix);
            };
                ...
                if (
                    ...
                    Object.keys(obj).forEach(function (key) {
                        obj[key] = bigNumberToString(
                            obj[key],
                            base,
                            ['increase', 'decrease', 'holdings', 'asset_amount'].includes(key)
                                ? 18
                                : places
                        );
                    });
            ...
            const getFeeAmount = ({
                ...
                transaction
                    .get('fee_tx_ids', Object(external_immutable_['List'])())
                    .forEach((fee_tx_id) => {
                        ...
                        entries.get('disposals').forEach((feeDisposal) => {
                            taxableFeeAmount = external_bignumber_js_['BigNumber'].sum(
                                taxableFeeAmount,
                                feeDisposal.get('proceedsAmount')
                            );
                        });
                    });
            ...
            const transactionHasFee = (transaction) => {
                return (
                    Boolean(transactionFeeCode(transaction)) &&
                    Boolean(transaction.get('fee_amount'))
                );
            };
            const transactionUnixNumber = (transaction) => {
                const txMoment = external_moment_default.a.utc(
            ...
            const getPriceBigNumber = (priceList, baseCode, quoteCode) => {
                ...
                try {
                    const stringPrice = priceList
                        .find(
                            (p) =>
                                p.get('base_code') === baseCode && p.get('quote_code') === quoteCode
                        )
                        .get('price');
            ...
            const lotsAndDisposalsFromTrade = ({
                ...
                const tradeAmount = new external_bignumber_js_['BigNumber'](
                    transaction.get(`${priceStrategy}_amount`)
                );
                ...
                if (isBuy) {
                    basisAmount = external_bignumber_js_['BigNumber'].sum(
                        basisAmount,
                        taxableFeeAmount
                    );
                } else {
                ...
                const lotAmount = new external_bignumber_js_['BigNumber'](
                    transaction.get(`${lotSide}_amount`)
                );
                const taxLots = Object(external_immutable_['List'])([
                    new src_taxLot({
                        unix: unixNumber,
                        assetCode: lotCode,
                ...
                const disposalAmount = new external_bignumber_js_['BigNumber'](
                    transaction.get(`${disposalSide}_amount`)
                );
                const disposals = Object(external_immutable_['List'])([
                    new src_disposal({
                        unix: unixNumber,
            ...
            const lotsAndDisposalsFromDeposit = ({
                ...
                let lotAmount = new external_bignumber_js_['BigNumber'](
            ...
            const lotsAndDisposalsFromWithdrawal = ({
                ...
                let withdrawalAmount = new external_bignumber_js_['BigNumber'](
                    transaction.get('withdrawal_amount')
                );
                const withdrawalPrice = getPriceBigNumber(
                    transactionPrices,
                    withdrawalCode,
                    localCurrency
                );
            ...
            const lotsAndDisposalsFromCompoundRepay = ({
                ...
                const updatedTransactionsMap = transactionsMap.set(
                    ...
                    Object(external_immutable_['Map'])({
                        ...
                        fee_tx_ids: repayment.get(
                            'fee_tx_ids',
                            Object(external_immutable_['List'])()
                        ),
                    })
                );
                return lotsAndDisposalsFromWithdrawal({
            ...
            const accountingFuncFromTransaction = (transaction) => {
                const TYPE_TO_FUNCTION = {
                    ...
                    COMPOUND_LIQUIDATEBORROW_BORROWER:
                        lotsAndDisposalsFromCompoundLiquidateBorrower,
                    COMPOUND_LIQUIDATEBORROW_LIQUIDATOR:
                        lotsAndDisposalsFromCompoundLiquidateLiquidator,
                };
            ...
            const makeLotsAndDisposals = ({ prices, transactions, priceMethod, localCurrency }) => {
                const pricesMap = groupBy(prices, (t) => t.get('tx_id'));
                const transactionsMap = Object(external_immutable_['Map'])(
                    transactions.map((t) => [t.get('tx_id'), t])
                );
                return transactions.reduce((reduction, transaction) => {
                    ...
                    return reduction
                        .update('taxLotList', (existingList) =>
                            existingList.merge(entries.get('taxLots'))
                        )
                        .update('disposalList', (existingList) =>
            ...
            const buildReport = (options) => {
                ...
                const output = reportIterable.reduce(
                    (acc, iterableYear) => {
                        const reportYearSetup = acc.get('report').set(
                            iterableYear.get('year'),
                            Object(external_immutable_['fromJS'])({
                                income: [],
                                long: [],
                                short: [],
                                unmatched: [],
                        ...
                        const report = updateReportHoldingsFromLots(
                            resultingReportAndLots,
                            iterableYear.get('year')
                        );
                        return acc
                            .set('report', report)
                            .set('lots', resultingReportAndLots.get('lots'));
                    },
                    Object(external_immutable_['Map'])({
                        lots: taxLotsByAsset,
            ...
            const buildReportYear = (options) => {
                const { disposals, lots, report, costBasisMethod, localCurrency, allowLotOverlap } =
                    options;
                ...
                return sortedDisposals.reduce(
                    (acc, disposal) => {
                        ...
                        const disposalLots = acc
                            .get('lots')
                            .get(disposalAsset, Object(external_immutable_['List'])());
            ...
            const matchLots = (options) => {
                ...
                while (currentDisposal.assetAmount > new external_bignumber_js_['BigNumber'](0)) {
                    if (lotStack.size < 1) {
                        ...
                        if (
                            currentDisposal.assetAmount.isGreaterThanOrEqualTo(
                                lotStack.first().assetAmount
                            )
                        ) {
                            const res = exhaustLot(
                                currentReport,
                                currentDisposal,
                                lotStack,
                                localCurrency
            ...
            /* concated harmony reexport createReport */ __webpack_require__.d(
                __webpack_exports__,
                'createReport',
                function () {
                    return createReport;
